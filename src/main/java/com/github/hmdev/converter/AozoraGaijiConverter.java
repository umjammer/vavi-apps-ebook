...
public class AozoraGaijiConverter
{
    ...
    public AozoraGaijiConverter(String jarPath) throws IOException
    {
        //初期化
        //ファイルチェック取得 IVS優先
        this.loadChukiFile("/chuki_ivs.txt", chukiUtfMap);
        this.loadChukiFile("/chuki_utf.txt", chukiUtfMap);
        this.loadChukiFile("/chuki_alt.txt", chukiAltMap);
    }

    /** 注記変換ファイル読み込み 
     * @throws IOException */
    private void loadChukiFile(String srcFile, Map<String, String> chukiMap) throws IOException
    {
        BufferedReader src = new BufferedReader(new InputStreamReader(AozoraGaijiConverter.class.getResourceAsStream(srcFile), "UTF-8"));
        String line;
        int lineNum = 0;
        try {
